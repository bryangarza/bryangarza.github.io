#+TITLE: Home
#+AUTHOR: Bryan Garza
#+DATE: [2016-01-01 Fri]
#+EMAIL: brygarza@gmail.com
#+OPTIONS: title:nil H:5

I'm a computer scientist interested in functional programming. My favorite
programming languages are OCaml, Haskell, Racket, and Emacs Lisp. ([[https://github.com/bryangarza][Github]])

* All Posts @@html:<span><a href="http://bryangarza.github.io/archive.html">(Archive)</a></span>@@
#+BEGIN_HTML
<div class="outline-3"><h3><a href="./on-the-importance-of-enforcing-invariants.html">On The Importance Of Enforcing Invariants</a><span> </span><span class="timestamp-wrapper"><span class="timestamp">(February 4, 2016)</span></span></h3>
#+END_HTML
#+INCLUDE: "on-the-importance-of-enforcing-invariants.org" :lines "11-"
#+BEGIN_HTML
</div>
#+END_HTML

#+BEGIN_HTML
<div class="outline-3">
<h3><a href="./universal-quantification-in-ocaml.html">Universal Quantification in OCaml</a> <span class="timestamp-wrapper"><span class="timestamp">(February 2, 2016)</span></span></h3>
#+END_HTML
#+INCLUDE: "universal-quantification-in-ocaml.org" :lines "11-"
#+BEGIN_HTML
</div>
#+END_HTML

#+BEGIN_HTML
<div class="outline-3"><h3><a href="./diff-hl-replacing-git-gutter.html">Diff-hl replacing git-gutter</a><span> </span><span class="timestamp-wrapper"><span class="timestamp">(January 31, 2016)</span></span></h3>
#+END_HTML
#+INCLUDE: "diff-hl-replacing-git-gutter.org" :lines "11-"
#+BEGIN_HTML
</div>
#+END_HTML

#+BEGIN_HTML
<div class="outline-3"><h3><a href="./automating-blog-posts-with-org-mode.html">Automating Blog Posts With Org-mode</a><span> </span><span class="timestamp-wrapper"><span class="timestamp">(January 31, 2016)</span></span></h3>
#+END_HTML
#+INCLUDE: "automating-blog-posts-with-org-mode.org" :lines "11-"
#+BEGIN_HTML
</div>
#+END_HTML

#+BEGIN_HTML
<div class="outline-3"><h3><a href="./basic-graph-traversal-in-ocaml.html">Basic Graph Traversal in OCaml</a><span> </span><span class="timestamp-wrapper"><span class="timestamp">(January 28, 2016)</span></span></h3>
#+END_HTML
#+INCLUDE: "basic-graph-traversal-in-ocaml.org" :lines "11-"
#+BEGIN_HTML
</div>
#+END_HTML
